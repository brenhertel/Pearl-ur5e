...
11/3/2021: pushed new changes to repository. Was running into an issue where /joint_states, /tf, and /wrench would publish at a rate of about 500 Hz, but the gripper position would publish at a rate of about 166 Hz. Due to this, the times of the messages would never line up exactly and no data was recorded. I fixed this by modifying the demo recorder to record times if they were within 0.001 s of each other, leading to a demo recording rate of about 166 Hz. Also fixed a bug which made the first line of all the data recorded 0's. Finally, added a script called plot_v4_demo.py which plots the data stored in a .h5 file from the newest demo recorder. Can be used to visualize and verify captured data.

3/18/2022: pushed new changes to repository. Pushed changes included in the development of the camera for the UR5e workspace. Updates include a camera publisher. (NOTE: only publishes image data, does not publish depth data. Depth functionality will be added in the future.) Added SIFT for object detection, a node which transforms the camera (x, y) into the robot (x, y) using coordinate transforms. (Note: the coordinate transform is only accurate to about 1 cm. I need to add in the gripper to change the tool0 location, then redo the coordinate transform. Additionally, adding in depth would make the transform more accurate, right now it takes (x, y) -> (x, y).) Attempted to add in functionality for the robot to follow a detected object, but so far this has been unsuccessful. Will continue to work and improve on this.

3/28/2022: pushed new changes to repository. Continued work on camera development. Added in color filtering for object detection, kalman filter for object localization, and an attempt at a demo using the cup (this demo has been largely unsuccessful so far).

5/24/2022: pushed new changes to repository. Since last changes, more camera support has been added. Newest demo uses PID controllers to follow the red cup with camera mounted on the robot arm. Note that this functionality (use cup_follower.launch and pid_follower.py) does not use coordinate transforms, but the PID inputs are in camera pixels. Depth functionality for the camera is not yet working. There seems to be some package issue with the sdk provided by intel which needs to be resolved. Change SIFT functionality to find screwdrivers. Added camera_save.py, which records video from the camera publisher (additionally can annotate with object detection and some basic filtering). Added tf_tool.py, which is an attempt to perform the coordinate transformations from the last robot joint (the output of /tf) to the end effector (this would be useful when recording demonstrations). The coordinate transform does not currently work.

8/22/2022: pushed new changes to repository. Was able to get `pyrealsense2` working, which is now being used to access the camera instead of opencv as it can integrate with the depth module. However, pyrealsense2 only works with python3.9 (ans was also built from source in the librealsense folder). To publish camera data, a script called `camera_full_pub.py` (https://github.com/brenhertel/Pearl-ur5e/blob/master/brendan_ur5e/src/scripts/camera_full_pub.py) was created. Note that this must be run with python3.9. This publishes a custom image-like message. Another script, `camera_converter.py` (https://github.com/brenhertel/Pearl-ur5e/blob/master/brendan_ur5e/src/scripts/camera_converter.py) is used to put these back into python2.7 language, with the use of CvBridge. With these two scripts running, the output topics `/camera/rgb/image_raw` and `/camera/depth/image_raw` are created. Note that the rgb camera and the depth camera in the intel RealSense D435i are not the same FOV and not in the exact same place, so the pixel values of each do not correspond to the same point in space. I tried to correct the FOV issue with the `/camera/depth/image_resize` topic but this does not account for the shift in space, and is still inacurate. I may try and remedy this in the future, or use other methods in order to find a mapping between (x, y) pixel values of the two cameras. The camera is still not in a permanent location in the lab setup, and this shouldn't be worried about until it is. Additionally, added the work of Joel (ur5e_natural_motion) to create dynamic motion planning using joint control provided with moveit, and Nakul (Towers-of-Hanoi) for goal-based planning and execution.

8/23/2022: pushed new changes to repository. Seperated out camera_converter to camera_rgb_converter and camera_depth_converter. Added both into launch file `full_camera.launch`, which will bring up all nodes necessary for recieving data from a single realsense camera. Additional changes made to Towers of Hanoi project.

9/28/2022: pushed new changes to repository. Added second camera to lab setup, with the `multicam.launch` file used to launch the readers/publishers and converters for both cameras. RGB has pretty low latency, but for some reason depth is a bit choppy. To reduce lag, each publisher is in a seperate file, so multicam.launch starts 6 scripts. Still need to perform homography for the new setup.

11/3/2022: pushed new changes to repository. Added in scripts to recieve object data from the gripper. The gripper detects objects if the target open/close position does no match the actual open/close position. This is not the most accurate method, as it misses a lot of cases where closing the gripper in intervals of about 10% and stopping once the object is firmly grasped, and only really works well when trying to close the gripper fully and it is stopped short. Other methods, such as seeing voltages/torques on the motors controlling the gripper, may be investigated in the future.
