...
class MoveGroupPythonInterface(object):
  ...
  def plan_cartesian_path(self, scale=1):
    ...
    filename = '/home/bhertel/catkin_ws/h5 files/bad preprocessed recorded_demo Fri Jul  3 14:45:33 2020.h5'
    ...
    ndims, npts = np.shape(pos_rot_data)

    task_space_data = deform_demonstrarions.get_open_house_demo()

    #new_pos_x = mlfd.downsample_traj(np.reshape(pos_rot_data[0], (npts, 1)))
    #new_pos_y = mlfd.downsample_traj(np.reshape(pos_rot_data[1], (npts, 1)))
    #new_pos_z = mlfd.downsample_traj(np.reshape(pos_rot_data[2], (npts, 1)))
    #new_rot_x = mlfd.downsample_traj(np.reshape(pos_rot_data[3], (npts, 1)))
    #new_rot_y = mlfd.downsample_traj(np.reshape(pos_rot_data[4], (npts, 1)))
    #new_rot_z = mlfd.downsample_traj(np.reshape(pos_rot_data[5], (npts, 1)))
    #new_rot_w = mlfd.downsample_traj(np.reshape(pos_rot_data[6], (npts, 1)))

    #pos_rot_data = np.transpose(np.hstack((new_pos_x, new_pos_y, new_pos_z, new_rot_x, new_rot_y, new_rot_z, new_rot_w)))
...
#    plt.show()
    ...
    pos_rot_data[0] = task_space_data[:, 0]
    pos_rot_data[1] = task_space_data[:, 1]
    pos_rot_data[2] = task_space_data[:, 2]
    ...
    key_rots = R.from_quat([ [pos_rot_data[3][0],  pos_rot_data[4][0],  pos_rot_data[5][0],  pos_rot_data[6][0]], [0.627, 0.568, -0.378, -0.376] ])
    
    print(key_rots.as_quat())
    t1 = 0
    t2 = len(pos_rot_data[3]) - 1
    #t2 = 99
