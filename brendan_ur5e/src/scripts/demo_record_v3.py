...
def wr_callback(wrmsg, (tfmsg, jsmsg, arr_struct, start_time)):
	global recording
	#print("start_time: %d, msg_time: %d", start_time, jsmsg.header.stamp.secs)	
	if jsmsg.header.stamp.secs == wrmsg.header.stamp.secs and jsmsg.header.stamp.nsecs == wrmsg.header.stamp.nsecs and jsmsg.header.stamp.secs >= start_time and recording == 1:
...
def tf_callback(tfmsg, (jsmsg, arr_struct, start_time)):	
	if jsmsg.header.stamp.secs == tfmsg.transforms[0].header.stamp.secs and jsmsg.header.stamp.nsecs == tfmsg.transforms[0].header.stamp.nsecs and tfmsg.transforms[0].child_frame_id == 'tool0_controller':
		#global sub_wr
		sub_wr = rospy.Subscriber("wrench", WrenchStamped, wr_callback, (tfmsg, jsmsg, arr_struct, start_time))

#just subscribes to tf and passes that callback the joint state message, and thus the data contained in it
def js_callback(jsmsg, (arr_struct, start_time)):
	#global sub_tf
	sub_tf = rospy.Subscriber("tf", tfMessage, tf_callback, (jsmsg, arr_struct, start_time))
...
#	grip_status_arr = np.empty([1])
	...
	sub_js = rospy.Subscriber("joint_states", JointState, js_callback, (arr_struct, start_time))
	...
	end_srvc = rospy.Service('end_demo', shutdown_request, shutdown_self)
...
#	grip_status_arr = np.delete(grip_status_arr, 0)
	...
	if (cont == 'y'):
		#rospy.signal_shutdown('resetting')
		pre_start()		
		#req = request()
		#begin_listener(req)
...
def pre_start():
	#time.sleep(20)
	#create the node
	rospy.init_node('demo_recorder', anonymous=True, log_level=rospy.INFO, disable_signals=False)
	start_srvc = rospy.Service('start_demo', shutdown_request, begin_listener)	
