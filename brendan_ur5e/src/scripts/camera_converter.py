...
class CameraConverter(object):
    ...
    def __init__(self):
        self.pub_rgb = rospy.Publisher('/camera/rgb/image_raw', Image, queue_size=1)
    ...
    def camera_conversion_rgb(self, msg):
    
        b = bytearray()
        b.extend(map(ord, msg.data))
        new_data = list(b)
        #print(new_data[0:3])
        self.pub_rgb.publish(self.bridge.cv2_to_imgmsg(np.array(new_data, dtype = np.uint8).reshape((msg.height, msg.width, msg.length)), msg.encoding))
    ...
    def camera_conversion_depth(self, msg):
        ...
        self.pub_depth.publish(self.bridge.cv2_to_imgmsg(img_data, msg.encoding))
        #image_arr = np.array(new_data, dtype = np.uint8).reshape((msg.height, msg.width, msg.length))
        #print(image_arr)
        #cv2.imshow("frame", image_arr)
        #cv2.waitKey(10)
...
if __name__ == '__main__':
    ...
    sub_rgb = rospy.Subscriber('/camera/rgb/image_muddy', image_data, cc.camera_conversion_rgb)
    sub_rgb = rospy.Subscriber('/camera/depth/image_muddy', image_data, cc.camera_conversion_depth)
    rospy.spin()
