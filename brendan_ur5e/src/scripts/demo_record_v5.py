...
def end_record(js_file, tf_file, wr_file, gr_file, cm_file):
    print(time.time())
...
def demo_recorder():
    ...
    try:
    	...
    	#create camera data file
    	cm_fp = open('camera_data.txt', 'w')
    	...
    	sub_cm = rospy.Subscriber("/camera/rgb/image_raw", Image, cm_callback, cm_fp)
    		
    	rospy.loginfo('Recording has started')
    		
    	rospy.spin()
       	end_record(js_fp, tf_fp, wr_fp, gr_fp, cm_fp)
