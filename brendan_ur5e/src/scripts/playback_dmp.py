...
sys.path.insert(1, '/home/bhertel/catkin_ws/src/brendan_ur5e/src/scripts/dmp_for_comparison/')
...
class MoveGroupPythonInterface(object):
  ...
  def plan_cartesian_path(self, scale=1):
    ...
    for i in range (3):#np.shape(pos_rot_data)[0]):
      ...
      #print(end)
      #print(indeces)
      pos_rot_data[i] = np.reshape(pnd.perform_new_dmp_adapted(pos_rot_data[i], initial, end), np.shape(pos_rot_data[i]))
    print('After DMP Deformation')
    #print(np.shape(pos_rot_data))
    for i in range (7):
      print(pos_rot_data[i][0])
